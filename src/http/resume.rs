...
pub async fn check_resume(
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|e| EngineError::protocol(ProtocolErrorKind::InvalidResponse, format!("HEAD request failed: {}", e)))?;
...
pub async fn verify_range_support(
    client: &Client,
    url: &str,
    user_agent: &str,
) -> Result<bool> {
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|e| EngineError::protocol(ProtocolErrorKind::InvalidResponse, format!("Range request failed: {}", e)))?;
...
pub fn validate_resumed_position(
    expected_start: u64,
    content_range: &str,
) -> Result<()> {
...
pub async fn cleanup_partial(part_path: &Path) -> Result<()> {
    if part_path.exists() {
        fs::remove_file(part_path).await.map_err(|e| {
            EngineError::Internal(format!("Failed to remove partial file: {}", e))
        })?;
    }
    Ok(())
}
